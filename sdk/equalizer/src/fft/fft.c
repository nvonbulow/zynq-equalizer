...
static void fft_commit_params(fft_t* p_fft_inst)
{
	...
	nfft |= (int_log2(p_fft_inst->num_pts) << FFT_NUM_PTS_SHIFT)   & FFT_NUM_PTS_MASK;

	// NFFT is channel 1 bits [4:0]
	XGpio_DiscreteWrite(&p_fft_inst->periphs.gpio_inst, 1, nfft);
	// fwd_inv is channel 2 bits [0:0]
	// scale_sch is channel 2 bits [26:1]
	XGpio_DiscreteWrite(&p_fft_inst->periphs.gpio_inst, 2, reg);
